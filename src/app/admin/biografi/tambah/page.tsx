...
import { createBiografi } from "@/lib/api/biografi";
import { CreateBiografiData, BiografiTranslation } from "@/types/biografi";
...
export default function TambahBiografiPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData: CreateBiografiData = {
        translations: translationsArray,
      };

      await createBiografi(formData);
      router.push("/admin/biografi");
    } catch (err: any) {
      console.error("Error creating biografi:", err);
      const errorMessage =
        err.response?.data?.message || err.message || "Gagal membuat biografi";
    ...
    <div className="container mx-auto px-4 py-16 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Buat biografi baru dengan dukungan multi-bahasa (Indonesia, English,
          Русский)
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {loading ? "Menyimpan..." : "Simpan Biografi"}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
