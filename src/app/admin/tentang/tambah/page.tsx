...
export default function TambahTentangPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData: CreateTentangData = {
        translations: translationsArray,
        foto: fotoFile || undefined,
      };

      await createTentang(formData);
      router.push("/admin/tentang");
    } catch (err: any) {
      console.error("Error creating tentang:", err);
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Gagal membuat data tentang";
    ...
    <div className="container mx-auto px-4 py-16 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Tambah Data Tentang Baru
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Buat data tentang baru dengan dukungan multi-bahasa (Indonesia,
          English, Русский)
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {loading ? "Menyimpan..." : "Simpan Data Tentang"}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
