...
export default function EditPengalamanPage() {
  ...
  const fetchPengalaman = async () => {
    try {
      ...
      const [idData, enData, ruData] = await Promise.all([
        getPengalamanById(id, "id").catch(() => null),
        getPengalamanById(id, "en").catch(() => null),
        getPengalamanById(id, "ru").catch(() => null),
      ]);

      // Set data dari data pertama yang ada
      const firstData = idData || enData || ruData;
      if (firstData) {
        setDurasi(firstData.durasi || "");
      }
      ...
      setTranslations({
        id: idData
          ? {
              language_code: "id",
              posisi: idData.posisi || "",
              instansi: idData.instansi || "",
            }
          : { language_code: "id", posisi: "", instansi: "" },
        en: enData
          ? {
              language_code: "en",
              posisi: enData.posisi || "",
              instansi: enData.instansi || "",
            }
          : { language_code: "en", posisi: "", instansi: "" },
        ru: ruData
          ? {
              language_code: "ru",
              posisi: ruData.posisi || "",
              instansi: ruData.instansi || "",
            }
          : { language_code: "ru", posisi: "", instansi: "" },
      });

      // Set kegiatans - fetch untuk semua bahasa
      if (firstData?.kegiatans && firstData.kegiatans.length > 0) {
        // Fetch pengalaman untuk semua bahasa untuk mendapatkan kegiatan translations
        const [idFullData, enFullData, ruFullData] = await Promise.all([
          getPengalamanById(id, "id").catch(() => null),
          getPengalamanById(id, "en").catch(() => null),
          getPengalamanById(id, "ru").catch(() => null),
        ]);

        const formattedKegiatans = firstData.kegiatans.map((kegiatan) => {
          const idKegiatan = idFullData?.kegiatans?.find(
            (k) => k.id === kegiatan.id
          );
          const enKegiatan = enFullData?.kegiatans?.find(
            (k) => k.id === kegiatan.id
          );
          const ruKegiatan = ruFullData?.kegiatans?.find(
            (k) => k.id === kegiatan.id
          );
          ...
          return {
            ...
            translations: {
              id: {
                language_code: "id",
                kegiatan: idKegiatan?.kegiatan || "",
              },
              en: {
                language_code: "en",
                kegiatan: enKegiatan?.kegiatan || "",
              },
              ru: {
                language_code: "ru",
                kegiatan: ruKegiatan?.kegiatan || "",
              },
            },
          };
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData: UpdatePengalamanData = {
        durasi: durasi.trim() || undefined,
        translations: translationsArray,
        kegiatans: kegiatansArray,
      };

      await updatePengalaman(id, formData);
      router.push("/admin/pengalaman");
    } catch (err: any) {
      console.error("Error updating pengalaman:", err);
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Gagal mengupdate pengalaman";
    ...
    <div className="container mx-auto px-4 py-16 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Edit informasi pengalaman dengan dukungan multi-bahasa
        </p>
      </div>
