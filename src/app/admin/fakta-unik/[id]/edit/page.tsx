...
import { UpdateFaktaUnikData, FaktaUnikTranslation } from "@/types/faktaUnik";
...
export default function EditFaktaUnikPage() {
  ...
  const fetchFaktaUnik = async () => {
    try {
      ...
      const [idData, enData, ruData] = await Promise.all([
        getFaktaUnikById(id, "id").catch(() => null),
        getFaktaUnikById(id, "en").catch(() => null),
        getFaktaUnikById(id, "ru").catch(() => null),
      ]);

      // Set angka dari data pertama yang tersedia
      const firstData = idData || enData || ruData;
      if (firstData) {
        setAngka(firstData.angka);
      }

      // Set translations untuk setiap bahasa (termasuk satuan)
      setTranslations({
        id: idData
          ? {
              language_code: "id",
              satuan: idData.satuan || "",
              isi: idData.isi || "",
            }
          : { language_code: "id", satuan: "", isi: "" },
        en: enData
          ? {
              language_code: "en",
              satuan: enData.satuan || "",
              isi: enData.isi || "",
            }
          : { language_code: "en", satuan: "", isi: "" },
        ru: ruData
          ? {
              language_code: "ru",
              satuan: ruData.satuan || "",
              isi: ruData.isi || "",
            }
          : { language_code: "ru", satuan: "", isi: "" },
      });
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData: UpdateFaktaUnikData = {
        angka,
        translations: translationsArray,
      };

      await updateFaktaUnik(id, formData);
      router.push("/admin/fakta-unik");
    } catch (err: any) {
      console.error("Error updating fakta unik:", err);
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        "Gagal mengupdate data fakta unik";
    ...
    <div className="container mx-auto px-4 py-16 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Edit Data Fakta Unik
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Edit data fakta unik dengan dukungan multi-bahasa (Indonesia, English,
          Русский)
        </p>
      </div>
